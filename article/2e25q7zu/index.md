---
url: /rui-plume/article/2e25q7zu/index.md
---
\[\[toc]]
...
5. 自动化调度：采用PowerJob Xxl-Job实现自动化任务调度，如定期备份配置、自动更新固件等。
6. 前端页面展示：使用Jquery+EasyUI+Twaver框架进行前端页面的构建与设计，优化页面显示。
7. 主要负责和产品经理对接需求，负责项目需求分析、架构设计、任务分工。
8. 和测试运维同事完成产品测试发版并负责后期项目的维护和升级.负责对项目架构进行优化设计。

二、 智能仓管（https://gitee.com/jishenghua/JSH\_ERP?\_from=gitee\_search） 进销存管理
项目周期：8 个月
开发环境：Maven+Idea+Jdk1.8+Svn
...
三、智能售货（https://github.com/ilovend/lkd\_parent）  https://blog.csdn.net/fegus/article/details/124205999
项目周期：9 个月
开发环境：Maven+Idea+Jdk1.8+Svn
...
6. 完成修改MAC地址扫描：4个网段同时扫描性能不够，一次需要花费10-20分钟，目前在4分钟左右（目前在4分钟左右）

7. 优化mac地址扫描，将后端返回数据由一次性全部返回，改为一条一条返回，避免等待时间过长。

8. 点击寻呼，未选中管理域或设备时操作为创建管理域+寻呼，选中管理域操作为寻呼，选中设备操作为刷新设备。

9. 调查ablite新打包，安装运行时闪退问题。

10. 调查添加一台设备寻呼时拓扑会乱。

11. 完成增加拓扑定位接口，用于定位拓扑的位置。

12. 完成修改拓扑中有10台设备，添加一台设备会乱，用户原来怎样就怎样

13. 完成修改一键拓扑逻辑，使设备全部更新完毕后才能进行拓扑监控。

14. 优化设备告警表格做大一点

15. 左侧设备列表区块，拉动的太窄导致按钮显示不全，建议改进

16. 优化面板页面添加一个刷新按钮，端口updown状态变化的时候可以显示

17. 整理调查测试部提出的问题，编写excel文档。

18. 完成修改7228B设备升级问题，文件过大，设备返回数据需要等待8分钟左右才能升级成功。

19. 调查w3305设备，固件升级问题，abview本地跑不报错，未找到相关原因。

20. 调查w3305设备，固件升级问题。

21. 打包3.5.8版本，并进行测试编写文档。
